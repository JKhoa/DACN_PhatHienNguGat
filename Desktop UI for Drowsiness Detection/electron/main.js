...
function startPythonBackend() {
    ...
    // Use enhanced server that exposes /detection and annotated frames
    // Use enhanced tracking backup if available, otherwise fallback to server_enhanced.py
    const fs = require('fs');
    let serverPath = path.join(pythonPath, 'server_with_tracking_backup.py');
    if (!fs.existsSync(serverPath)) {
      serverPath = path.join(pythonPath, 'server_enhanced.py');
      console.log('Using default server_enhanced.py (tracking backup not found)');
    } else {
      console.log('✅ Using enhanced tracking backup server (supports 20+ students)');
    }
    const reqsPath = path.join(pythonPath, 'requirements.txt');

    // Prefer workspace virtualenv if present
