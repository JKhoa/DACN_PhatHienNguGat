...
export function LogPanel({ logs, cameras, onExport }: LogPanelProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch today stats
        const todayRes = await fetch(`http://127.0.0.1:5000/api/logs/summary?period=today`);
        if (todayRes.ok) {
        ...
        // Fetch weekly stats
        const weekRes = await fetch(`http://127.0.0.1:5000/api/logs/summary?period=week`);
        if (weekRes.ok) {
        ...
        // Fetch monthly stats
        const monthRes = await fetch(`http://127.0.0.1:5000/api/logs/summary?period=month`);
        if (monthRes.ok) {
        ...
        const camerasRes = await fetch(`http://127.0.0.1:5000/api/logs/cameras`);
        if (camerasRes.ok) {
          ...
          for (const cam of cameraList) {
            try {
              const logsRes = await fetch(`http://127.0.0.1:5000/api/logs/events/${cam.id}?period=today`);
              if (logsRes.ok) {
