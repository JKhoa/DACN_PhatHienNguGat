...
export function CameraCard({
  ...
  // Throttle âm báo: lưu thời điểm beep gần nhất theo person id (ms)
  const lastAlertRef = useRef<Map<string, number>>(new Map());
  const audioCtxRef = useRef<AudioContext | null>(null);

  // Phát beep ngắn cảnh báo drowsy. Web Audio API thay vì <audio> để không cần asset.
  ...
  // Kiểm tra danh sách students mới, nếu có ai chuyển sang drowsy và chưa beep trong 30s gần đây thì beep.
  const checkAndAlertDrowsy = (students: Array<{ id: string; state: string }>) => {
    const now = Date.now();
    const THROTTLE_MS = 30000;
    let shouldBeep = false;
    for (const s of students) {
      if (s.state !== 'drowsy') continue;
      const last = lastAlertRef.current.get(s.id) || 0;
      if (now - last >= THROTTLE_MS) {
        lastAlertRef.current.set(s.id, now);
        shouldBeep = true;
      }
    }
    if (shouldBeep) playDrowsyBeep();
    // Dọn entry cũ (>5 phút) để tránh map phình
    if (lastAlertRef.current.size > 200) {
      for (const [pid, t] of lastAlertRef.current) {
        if (now - t > 5 * 60 * 1000) lastAlertRef.current.delete(pid);
      }
    }
  };
  ...
  useEffect(() => {
    ...
    if (camera.isRunning && camera.status === 'online') {
      ...
      // Theo dõi thay đổi kích thước qua ResizeObserver — không cần polling.
      ...
      // Fallback: window resize (cover trường hợp parent layout đổi mà canvas không đổi rect ngay)
  ...
  const drawOverlays = (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
    ...
    trackingBoxes.forEach((box: any) => {
      ...
      // --- DRAWING LOGIC V2 (UNIFIED) ---
      // Determine state and colors
      let boxColor = '#00e676'; // Mặc định: Xanh (Tỉnh táo)
      let labelText = 'TỈNH TÁO';
      let isViolating = false;

      if (box.state === 'drowsy' || box.state === 'sleeping') {
        boxColor = '#f59e0b'; // Vàng cam (Ngủ gật)
        labelText = 'NGỦ GẬT';
        isViolating = true;
      } else if (box.class_name === 'dien_thoai' || box.display_name?.toLowerCase().includes('điện thoại')) {
        boxColor = '#ef4444'; // Đỏ (Điện thoại)
        labelText = 'DÙNG ĐIỆN THOẠI';
        isViolating = true;
      }

      // Draw main bounding box with rounded effect
      ctx.strokeStyle = boxColor;
      ctx.lineWidth = 3;
      const cornerRadius = 8;

      // Vẽ khung bo góc thủ công trên Canvas
      ctx.beginPath();
      ...
      <div className="relative aspect-video bg-gray-800">
        ...
        {camera.isRunning && camera.status === 'online' ? (
          <div className="relative w-full h-full">
            ...
            {/* Loading overlay khi video chưa ready (webcam) hoặc IP camera chưa nhận frame nào */}
            {camera.type === 'webcam' && !localError && !videoStream && (
              <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/55 backdrop-blur-sm">
                <div className="text-center text-blue-100">
                  ...
                  <div className="font-semibold">Đang khởi động camera...</div>
                  <div className="text-xs opacity-80 mt-1">Cấp quyền webcam và chờ stream sẵn sàng</div>
                </div>
              </div>
            )}
            {camera.type !== 'webcam' && wsStudents.length === 0 && wsFps === 0 && (
              <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/45 backdrop-blur-sm">
                <div className="text-center text-blue-100">
                  <Loader2 className="h-10 w-10 mx-auto mb-3 animate-spin" />
                  <div className="font-semibold">Đang chờ stream từ backend...</div>
                  <div className="text-xs opacity-80 mt-1">Kết nối WebSocket và đồng bộ camera</div>
                </div>
              </div>
            )}
          ...
          /* Offline State - cải thiện UX với CTA rõ ràng */
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800">
            <div className="text-center text-gray-300 max-w-xs px-4">
              <AlertCircle className="h-12 w-12 mx-auto mb-3 text-gray-500" />
              <div className="text-lg font-semibold">Camera đang tắt</div>
              <div className="text-sm text-gray-400 mt-1">
                {camera.errorMessage || 'Nhấn "Bắt đầu" để kết nối và phát hiện ngủ gật'}
              </div>
              <Button
                size="sm"
                className="mt-4"
