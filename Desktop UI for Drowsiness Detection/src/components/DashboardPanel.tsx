...
export function DashboardPanel() {
  ...
  const fetchCameraDetail = async (cameraId: string) => {
    try {
      const response = await fetch(`http://localhost:5000/api/logs/events/${encodeURIComponent(cameraId)}?period=${period}`);
  ...
  const exportReport = async (format: 'pdf' | 'excel') => {
    try {
      const response = await fetch(`http://localhost:5000/api/logs/export/${format}`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `drowsiness_report_${period}.${format === 'pdf' ? 'pdf' : 'xlsx'}`;
