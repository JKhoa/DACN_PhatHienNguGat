...
export function DateRangeSettingsPanel({ cameraId, cameraName }: DateRangeSettingsPanelProps) {
  ...
  const fetchStatsForRange = async () => {
    ...
    try {
      ...
      const endpoint = cameraId 
        ? `http://localhost:5000/api/logs/stats/${cameraId}?period=${period}`
        : `http://localhost:5000/api/logs/summary?period=${period}`;
  ...
  const exportReport = async (exportFormat: 'pdf' | 'excel') => {
    ...
    try {
      ...
      const body = cameraId 
        ? { period, camera_ids: [cameraId] }
        : { period };
31
      const response = await fetch(`http://localhost:5000/api/logs/export/${exportFormat}`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `drowsiness_report_${startStr}_to_${endStr}.${exportFormat === 'pdf' ? 'pdf' : 'xlsx'}`;
