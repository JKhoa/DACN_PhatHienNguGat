...
def seed_sample_data():
    ...
    for days_ago in range(7):
        ...
        for i in range(num_events):
            ...
            # Insert event with custom start_time
            cursor = db._get_connection().cursor()
            cursor.execute('''
                INSERT INTO drowsy_events 
                (camera_id, camera_name, student_id, start_time, event_type, is_active)
                VALUES (?, ?, ?, ?, ?, 0)
            ''', (camera_id, camera_name, student_id, start_time.strftime('%Y-%m-%d %H:%M:%S'), 'drowsy'))
            
            event_id = cursor.lastrowid
            
            # Update end_time and duration
            cursor.execute('''
                UPDATE drowsy_events 
                SET end_time = ?, 
                    duration_seconds = ?
                WHERE id = ?
            ''', (end_time.strftime('%Y-%m-%d %H:%M:%S'), duration_seconds, event_id))
