...
class CameraLogger:
    ...
    def get_statistics(self, start_date: datetime, end_date: datetime) -> Dict:
        ...
        stats = {
            ...
            'total_duration_seconds': round(total_duration, 2),
            'total_duration_display': self._format_duration(total_duration),
            'most_drowsy_student': most_drowsy_student,
            'most_drowsy_duration': round(max_duration, 2) if most_drowsy_student else 0,
        }
...
class MultiCameraLogger:
    ...
    def get_summary_stats(self, period: str = 'today') -> Dict:
        """Thống kê tổng hợp tất cả các phòng"""
        all_stats = self.get_all_cameras_stats(period)
        
        total_students = sum(s.get('total_drowsy_students', 0) for s in all_stats)
        total_events = sum(s.get('total_events', 0) for s in all_stats)
        total_duration = sum(s.get('total_duration_seconds', 0) for s in all_stats)
        currently_drowsy = sum(s.get('currently_drowsy', 0) for s in all_stats)
